...
def mocked_run_next_task(queue=None):
    """
    We mock tasks.mocked_run_next_task to give other threads some time to update
    the database.
...
class TestBackgroundDecorator(TransactionTestCase):
    ...
    def test_str(self):
        proxy = tasks.background()(empty_task)
        self.assertEqual(
            u"TaskProxy(background_task.tests.test_tasks.empty_task)", str(proxy)
        )
...
class PriorityTestCase(TransactionTestCase):
    ...
    def test_priority(self):
        ...
        # Using a list here. QuerySet.last() is prohibited after slicing
        # (new in Django 2.0)
...
class DatabaseOutageTestCase(TransactionTestCase):
    ...
    def test_dropped_db_connection(self, mock_logger):
        ...
        with patch("django.db.backends.utils.CursorWrapper", cursor_wrapper):
            run_next_task()
