...
class Command(BaseCommand):
    ...
    # Command options are specified in an abstract way
    # to enable Django < 1.8 compatibility
    OPTIONS = (
        (
            ("--duration",),
            {
    ...
    if VERSION < (1, 8):
        ...
        option_list = BaseCommand.option_list + tuple(
            [make_option(*args, **kwargs) for args, kwargs in OPTIONS]
        )
    ...
    def run(self, *args, **options):
        ...
        while (duration <= 0) or (time.time() - start_time) <= duration:
            ...
            if not self._tasks.run_next_task(queue):
                ...
                # there were some tasks to process, let's check if there is more work to
                # do after a little break.
                time.sleep(
                    random.uniform(
